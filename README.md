...
erDiagram
    ...
    STAFF ||--o{ VITAL_SIGN : logs
    STAFF ||--o{ RISK_PREDICTION : reviews
...
| :--- | :--- | :--- |
| **Patient** | Core Domain | Demographic registry and ward classification; serves as the root entity for all patient-centric records. |
| **VitalSign** | Time-Series | NEWS2-compliant clinical inputs; includes `staffId` to maintain a strict audit trail of logging personnel. |
| **RiskPrediction** | Inference Store | Persists ML outcomes and SHAP-based explainability; tracks `staffId` to record clinical review and validation events. |
| **Staff** | IAM & RBAC | Manages system access (Admin/Doctor/Nurse) and training status; acts as the authoritative source for clinical workflow accountability. |
